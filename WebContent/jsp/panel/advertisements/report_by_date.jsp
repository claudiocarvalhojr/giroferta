		...
		type_report = '${type_report}';
		  ...
		  var type_report = '${type_report}';
			  data = google.visualization.arrayToDataTable([
   			    ['Anúncios', 'Cliques'],
   			    <c:forEach items="${list}" var="hcc">
   					['${hcc.advertisement_id.id}', '${hcc.id}'],
   				</c:forEach>
   			  ]);
...
<div>
	...
	<br />
		...
		<form action="${linkAdvertisementsReportByDate}" name="formReportByDate" method="POST">
...
<!-- 		<div id="columnchart_material" style="width: 100%; height: 400px;"></div>
 ...
 -->			
		<table class="table table-striped table-hover table-responsive">
			<thead>
				<tr>
					<th style="width: 20%"><fmt:message key="report.column.date" /></th>
					<th style="width: 10%"><fmt:message key="report.advertisement.column.id" /></th>
					<th style="width: 55%"><fmt:message key="report.advertisement.column.title" /></th>
<%-- 					<th style="width: 15%"><a href="${linkAdvertisementsReportByDate}?type_report=gross"><fmt:message key="report.advertisement.column.gross_click" /></a></th>
 ...
 --%>					
			...
			<tbody>
				<c:forEach items="${list}" var="hcc">
					<tr title="${hcc.advertisement_id.id} - ${hcc.advertisement_id.title}" onclick="location.href='${giroUrl}${hcc.advertisement_id.url}'" style="cursor: pointer;">
						<td><fmt:formatDate pattern="dd/MM/yyyy HH:mm:ss" value="${hcc.created_at}"/></td>
						<td>${hcc.advertisement_id.id}</td>
						<c:choose>
							<c:when test="${fn:length(hcc.advertisement_id.title) > 30}">
								<c:set var="stringAux" value="${fn:substring(hcc.advertisement_id.title, 0, 29)}" />
								<td>${stringAux} ...</td>
							</c:when>
							<c:otherwise>
								<td>${hcc.advertisement_id.title}</td>
							</c:otherwise>
						</c:choose>
<%-- 						<td>${advertisement.gross_click}</td>
 ...
 --%>						
						<td>
							<c:choose>
								<c:when test="${hcc.advertisement_id.status == 1}"><fmt:message key="report.advertisement.status1" /></c:when>
								<c:when test="${hcc.advertisement_id.status == 2}"><fmt:message key="report.advertisement.status2" /></c:when>
								<c:when test="${hcc.advertisement_id.status == 3}"><fmt:message key="report.advertisement.status3" /></c:when>
							</c:choose>
						</td>
					</tr>
				</c:forEach>
